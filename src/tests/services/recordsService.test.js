...
describe('recordsService — loadAllRecords', () => {
  ...
  it('modo list (padrão) usa limite 500 por entidade', async () => {
    await loadAllRecords();
    for (const n of ALL_RECORD_ENTITIES) {
      expect(entities[n].list).toHaveBeenCalledWith('-created_date', 500);
    }
  });
...
describe('recordsService — loadRecordsByObra', () => {
  it('filtra server-side por obra_id com limite 500 e normaliza', async () => {
