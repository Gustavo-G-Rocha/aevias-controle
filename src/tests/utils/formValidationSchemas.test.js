...
// ── Paridade: validateChecklistUsinaForm (Zod) vs validação manual ──────
// Espelha checklistValidation.test.js para garantir paridade de comportamento.
describe('validateChecklistUsinaForm (schema-driven)', () => {
  describe('ao salvar progresso (rascunho)', () => {
    ...
    it('inválido quando obra_id está vazio', () => {
      const result = validateChecklistUsinaForm({ obra_id: '' }, 'rascunho');
      expect(result.valid).toBe(false);
      expect(result.message).toContain('obra');
    });
  });
