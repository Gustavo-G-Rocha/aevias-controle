...
export const CopyIdButton = React.memo(({ id }) => {
  ...
  return (
    <button
      ...
      onClick={handleCopy}
      title={`Copiar ID: ${id}`}
      className="inline-flex items-center gap-1 text-[9px] font-mono bg-muted hover:bg-secondary/20/40 px-1.5 py-0 rounded transition-colors text-muted-foreground hover:text-foreground leading-none">
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="h-5 w-5 p-0 text-xs font-normal hover:bg-black/10 shrink-0">
          <Filter className={`w-3 h-3 ${hasFilter ? 'text-secondary' : ''}`} />
        </Button>
      </PopoverTrigger>
