/**
 * Seção de fotos (páginas fotográficas) do boletim.
 */
import React from 'react';
import { getLogoUrl, chunkArray, formatDate } from '@/utils/relatorioBoletimSondagemTradoUtils';

export default function BoletimFotos({ boletim, obra, regional }) {
  if (!boletim.fotos || boletim.fotos.length === 0) {
    return null;
  }
  ...
  return photoChunks.map((chunk, pageIndex) => (
    ...
    >
      <div className="w-full max-w-[190mm] mx-auto flex-grow flex flex-col">
        <header className="grid grid-cols-3 items-center border-b-2 border-gray-800 pb-2">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600">
              Boletim de Sondagem a Trado — {obra?.name || ''}
            </p>
          </div>
        ...
        <main
          className="flex-grow grid grid-cols-2 gap-4 mt-4"
        ...
        >
          {chunk.map((fotoUrl, fotoIndex) => (
            <div
              ...
              style={{ height: 'calc((100vh - 300px) / 3)' }}
            >
