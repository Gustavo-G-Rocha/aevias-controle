...
// formatDateTerra → usa formatDate de relatorioUtils
const formatDateTerra = (dateString) => formatDate(dateString) === 'N/A' ? '' : formatDate(dateString);
...
const DadosObraTerra = ({ regional, obra, checklist }) => (
  <>
    ...
    <div className="grid grid-cols-3 gap-x-3 gap-y-0.5 mb-0.5 p-1 rounded text-xs">
      ...
      <div><p className="font-bold">MATERIAL:</p><p>{checklist.material}</p></div>
      <div><p className="font-bold">EMPREITEIRA:</p><p>{checklist.empreiteira}</p></div>
      <div><p className="font-bold">ESTACA:</p><p>{checklist.estaca}</p></div>
      <div><p className="font-bold">INSPETOR FISC:</p><p>{checklist.inspetor_fiscal || 'N/A'}</p></div>
      <div><p className="font-bold">OBRA:</p><p>{obra?.name || 'N/A'}</p></div>
      <div><p className="font-bold">CAMADA:</p><p>{checklist.camada}</p></div>
...
export default function RelatorioChecklistTerraplanagem({ checklist, creatorUser, obra: obraProp, regional: regionalProp }) {
  ...
  const footerProps = {
    labName: checklist.inspetor_fiscal || checklist.laboratorista_name, labEmail: checklist.created_by, labCreatedDate: checklist.created_date,
