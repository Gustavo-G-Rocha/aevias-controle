      ...
      <Card className="bg-white/20 backdrop-blur-lg border-white/20">
        <CardHeader>
          ...
          <div className="flex flex-wrap gap-4 mt-1 items-center">
            <span className={`text-xs font-medium px-2 py-1 rounded-full ${chartPoints.length >= 3 ? 'bg-[#BFCF99]/40 text-[#00233B]' : 'bg-red-100 text-red-600'}`}>
              {chartPoints.length} ponto{chartPoints.length !== 1 ? 's' : ''} válido{chartPoints.length !== 1 ? 's' : ''}
              {chartPoints.length < 3 && ` — mínimo 3 para calcular`}
            </span>
            {parabola && (
              <>
                <p className="text-sm text-[#00233B]/80"><span className="font-semibold">γd máx: </span><span className="text-[#00233B] font-bold">{parabola.gamma_max.toFixed(4)} g/cm³</span></p>
                <p className="text-sm text-[#00233B]/80"><span className="font-semibold">w ótima: </span><span className="text-[#00233B] font-bold">{parabola.w_otima.toFixed(2)}%</span></p>
              </>
            )}
          </div>
        </CardHeader>
