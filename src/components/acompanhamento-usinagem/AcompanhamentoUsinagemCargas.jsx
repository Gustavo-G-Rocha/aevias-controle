    ...
    <Card className="bg-card border border-border">
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>Cargas Acompanhadas</CardTitle>
          {isEditable && (
            <Button onClick={adicionarCarga} size="sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse border border-border text-sm">
              <thead className="bg-muted/50">
                <tr>
                  <th className="border border-border p-2">Placa Caminhão</th>
                  <th className="border border-border p-2">Hora de Saída</th>
                  <th className="border border-border p-2">Peso (t)</th>
                  <th className="border border-border p-2">Temperatura (°C)</th>
                  <th className="border border-border p-2">Temperatura (°C)</th>
                  <th className="border border-border p-2">Observação</th>
                  {isEditable && <th className="border border-border p-2">Ações</th>}
                </tr>
              </thead>
              <tbody>
                {cargas.map((carga, index) => (
                  <tr key={`carga-${index}`} className={index % 2 === 0 ? 'bg-transparent' : 'bg-muted/20'}>
