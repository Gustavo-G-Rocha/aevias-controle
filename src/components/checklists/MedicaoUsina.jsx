            ...
            <table className="w-full border-collapse text-sm min-w-[800px]">
              <thead>
                <tr className="bg-muted">
                  <th className="border border-border px-2 py-2 text-left font-medium">Nº Ticket (NF)</th>
                  <th className="border border-border px-2 py-2 text-left font-medium">Placa</th>
                  <th className="border border-border px-2 py-2 text-center font-medium">Qtde (t)</th>
                  <th className="border border-border px-2 py-2 text-center font-medium">Volume (m³)</th>
                  <th className="border border-border px-2 py-2 text-center font-medium">Temp. (°C)</th>
                  <th className="border border-border px-2 py-2 text-left font-medium">Rodovia Destino</th>
                  <th className="border border-border px-2 py-2 text-left font-medium">Equipe</th>
                  <th className="border border-border px-2 py-2 text-left font-medium">Observações</th>
                  {!disabled && <th className="border border-border px-2 py-2 w-10"></th>}
                </tr>
              </thead>
              <tbody>
                {data.cargas.map((carga, index) => (
                  <tr key={carga.numero_ticket ? `ticket-${carga.numero_ticket}` : `carga-${index}`} className={index % 2 === 0 ? 'bg-card' : 'bg-muted/30'}>
                    ...
                    {!disabled && (
                      <td className="border border-border px-1 py-1 text-center">
                        <Button type="button" variant="ghost" size="sm" onClick={() => removeCarga(index)} className="h-8 w-8 p-0 text-red-500 hover:text-destructive hover:bg-red-50">
