    ...
    <Card className="bg-muted/30">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {equivalenteAreia.medicoes.map((m, idx) => (
            <div key={idx} className="border rounded-lg p-4 bg-background space-y-3">
              <p className="font-bold text-sm text-foreground">Medição {idx + 1}</p>
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  ...
                  <Input value={m.equivalente || ""} disabled className="bg-muted text-sm font-bold" />
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="flex items-center gap-4 pt-2 border-t">
          ...
          <Input value={equivalenteAreia.media || ""} disabled className="bg-muted text-sm font-bold max-w-[140px]" />
        </div>
      </CardContent>
    </Card>
  );
